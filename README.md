# Shopauskunft Integration für Shopware 6

Mit **Shopauskunft Integration für Shopware 6** kannst du das Vertrauen deiner Besucherinnen und Besucher in deinen Online-Shop schnell und einfach stärken. Die Installation ist in nur 5 Minuten erledigt und erfordert keine technischen Vorkenntnisse. Dank dieser Erweiterung bleibst du technisch immer auf dem neuesten Stand – ganz ohne zusätzlichen Wartungsaufwand.

Mit nur wenigen Klicks integrierst du Vertrauenselemente wie das Shopauskunft-Widget in deinen Online-Shop. Deine Kundinnen und Kunden erhalten nach dem Kauf automatisch eine Anfrage zur Bewertung.

> **Hinweis:** Um Shopauskunft Integration für Shopware 6 zu nutzen, benötigst du eine bestehende Shopauskunft-Mitgliedschaft.



<img src="https://mzmuda.de/shopware6-plugins/shopware-freelancer-shopsakunft.png" width="1200" alt="Shopware Shopsakunft">
---

## Highlights

- Intuitive Integration des Shopauskunft-Widgets
- Automatisiertes Sammeln von verifizierten Bewertungen deiner Kunden

---

## Installation 
### Bitte teste das Plugin zunächst auf einem Testsystem
1. Plugin herunterladen: http://mzmuda.de/shopware6-plugins/mzmudaShopAuskunftWidget-SW65-SW66.zip
2. Im Shopware-Adminbereich unter „Erweiterungen > Meine Erweiterungen“ das Plugin hochladen und installieren.
3. Plugin aktivieren.
4. Plugin-Konfigurationsbereich eintragen.
5. Widget und Bewertungseinladungen sind automatisch aktiv.


<img src="https://mzmuda.de/shopware6-plugins/shopware6-widget-configuration.png" width="1200" alt="Shopware 6  konfiguration">

---

## Voraussetzungen

- Shopware 6.5 und Shopware 6.6 
- Bestehende Shopauskunft-Mitgliedschaft ( shopauskunft.de -   https://affiliate.haendlerbund.de/go.cgi?pid=2164&wmid=604&cpid=1&prid=130&subid=&target=Shopauskunft_LP_P )

---

## Support

Gerne per e-mail: info@mzmuda.de

**Autor:**  

<img src="https://mzmuda.de/wp-content/uploads/2022/12/shopware-freelancer-logo-1.png" width="300" alt="Shopware Freelancer">

Mateusz Zmuda – Shopware Freelancer  
[https://mzmuda.de/](https://mzmuda.de/)

---

## Lizenz

Jedem Nutzer (natürliche oder juristische Person) wird das nicht-exklusive Recht eingeräumt, diese Software in unveränderter Form kostenlos unter den folgenden Bedingungen:

1. Die Software darf **nicht verändert**, zurückentwickelt oder in abgewandelter Form weiterverbreitet werden.
2. Die Weitergabe darf ausschließlich in der **unveränderten Originalform** erfolgen.
3. Der Urheberrechtshinweis sowie dieser vollständige Lizenztext müssen jeder Kopie beigefügt werden.
4. Der Name des Autors darf **nicht aus Benutzeroberflächen oder Dokumentationen entfernt** werden, sofern er dort enthalten ist.
5. Eine **kommerzielle und nicht-kommerzielle Nutzung** ist ausdrücklich erlaubt.
6. Die Software wird **ohne jegliche Gewährleistung** bereitgestellt („as is“). Der Autor übernimmt keine Haftung für direkte oder indirekte Schäden, die aus der Nutzung resultieren.

Mit der Nutzung oder Weitergabe dieser Software erklärt sich der Nutzer mit den oben genannten Bedingungen einverstanden.
Weitere Informationen findest du auf der Website des Autors.

---



Viel Erfolg beim Sammeln von Bewertungen und Steigern deines Shop-Vertrauens!
